...
function MenCollectionContent() {
  ...
  const loadProducts = async () => {
    try {
      ...
      if (data.success) {
        const mensProducts = data.products.filter((p: Product) => p.category === "men");
  ...
  return (
    <div className="min-h-screen bg-white pt-20">
      ...
      {/* Side Navigation - Similar to homepage */}
      {isSideNavOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute right-0 top-0 h-full w-96 bg-white transform transition-transform duration-500 ease-in-out">
            {/* Side nav content similar to homepage */}
          </div>
        </div>
      )}
      ...
      <section className="relative h-96 bg-black flex items-center justify-center">
        ...
        <div className="relative z-10 text-center text-white">
          <h1 className="text-6xl font-serif tracking-widest mb-4">MEN'S COLLECTION</h1>
          <p className="text-xl font-light tracking-wide">Luxury fashion for the modern gentleman</p>
        </div>
      </section>
